...
function dx = mass_spring_damper(t, x)
    dx(1) = x(2); // Velocidade
    dx(2) = (u - (b_sub/m) * x(2) - (k/m) * x(1)); // Aceleração com entrada degrau
endfunction
// Condições iniciais
x0 = [0; 0]; // Posição inicial = 1 m, Velocidade inicial = 0 m/s

// Simulação
t = 0:0.01:10;
x = ode(x0, 0, t, mass_spring_damper);
